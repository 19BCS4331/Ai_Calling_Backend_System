...
export function BodyParameterBuilder({
  ...
  const renderParameter = (param: BodyParameter, level: number = 0, parentId?: string) => {
    ...
    return (
      <div key={param.id} className={`${level > 0 ? 'ml-6 border-l border-gray-200 dark:border-white/10 pl-4' : ''}`}>
        <div className="flex items-start gap-2 py-2">
          ...
          {/* Drag handle */}
          <div className="pt-3 text-gray-300 dark:text-white/20">
          ...
          <div className="flex-1 grid grid-cols-12 gap-2">
            ...
            <div className="col-span-3">
              <input
                ...
                disabled={disabled}
                className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-lg text-gray-900 text-sm placeholder-gray-400 focus:outline-none focus:border-purple-500/50 dark:bg-white/5 dark:border-white/10 dark:text-white dark:placeholder-white/30 disabled:opacity-50"
              />
            </div>
            ...
            <div className="col-span-1 flex items-center justify-center">
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  ...
                  disabled={disabled}
                  className="w-4 h-4 rounded border-gray-300 dark:border-white/20 bg-white dark:bg-white/5 text-purple-500 focus:ring-purple-500/20"
                />
                <span className="text-xs text-gray-400 dark:text-white/50">Req</span>
              </label>
            </div>
            ...
            <div className="col-span-1 flex items-center justify-end">
              <button
                ...
                disabled={disabled}
                className="p-2 text-gray-300 dark:text-white/40 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-colors disabled:opacity-50"
              >
                <Trash2 size={16} />
              </button>
            </div>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <label className="block text-sm font-medium text-gray-600 dark:text-white/70">Request Body Parameters</label>
        <div className="flex items-center gap-3">
          {onToggleRawJson && (
            <button
              type="button"
              onClick={onToggleRawJson}
              className="text-sm text-gray-400 dark:text-white/50 hover:text-gray-600 dark:hover:text-white/70 transition-colors"
            >
      ...
      {showRawJson ? (
        <div>
          <textarea
            ...
            rows={10}
            disabled={disabled}
            className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:border-purple-500/50 dark:bg-white/5 dark:border-white/10 dark:text-white dark:placeholder-white/30 font-mono text-sm resize-none disabled:opacity-50"
          />
        ...
        <div className="flex items-center justify-center py-8 border border-dashed border-gray-200 dark:border-white/10 rounded-xl">
          <button
            type="button"
            onClick={() => addParameter()}
            disabled={disabled}
          ...
          <div className="border border-gray-200 dark:border-white/10 rounded-xl p-3 bg-white dark:bg-white/[0.02] shadow-sm dark:shadow-none">
          {/* Header */}
          <div className="grid grid-cols-12 gap-2 px-8 pb-2 border-b border-gray-100 dark:border-white/10 text-xs text-gray-400 dark:text-white/50">
