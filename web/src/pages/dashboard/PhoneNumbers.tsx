  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">Phone Numbers</h1>
          <p className="text-gray-500 dark:text-white/50">Manage your phone numbers and link them to agents</p>
        </div>
      ...
      {showAddForm && (
        ...
        >
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Add Phone Number Manually</h3>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-gray-600 dark:text-white/60 mb-2">Phone Number</label>
              <input
                type="text"
                value={newNumber}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 dark:bg-white/5 dark:hover:bg-white/10 rounded-xl font-medium text-gray-700 dark:text-white transition-all"
              >
                Cancel
              </button>
            </div>
      ...
      {phoneNumbers.length === 0 ? (
        <div className="bg-white border border-gray-200 dark:bg-white/[0.02] dark:border-white/5 rounded-2xl p-12 text-center shadow-sm dark:shadow-none">
          <Phone size={48} className="mx-auto text-gray-200 dark:text-white/20 mb-4" />
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">No Phone Numbers</h3>
        ...
        <div className="grid gap-4">
          {phoneNumbers.map((number, i) => (
            <motion.div
              ...
              className="bg-white border border-gray-200 dark:bg-white/[0.02] dark:border-white/5 rounded-xl p-6 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors shadow-sm dark:shadow-none"
            >
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <Phone size={20} className="text-purple-400" />
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{number.phone_number}</h3>
                    <span className="px-2 py-1 bg-gray-100 dark:bg-white/5 rounded text-xs text-gray-500 dark:text-white/60">
                      {number.country_code}
                    </span>
                  </div>
                  <div className="flex items-center gap-4 text-sm text-gray-400 dark:text-white/50">
                  ...
                  {number.agent ? (
                    <>
                      <div className="px-4 py-2.5 bg-green-500/10 border border-green-500/20 rounded-xl">
                        ...
                        <p className="text-sm font-medium text-gray-900 dark:text-white">{number.agent.name}</p>
                      </div>
                      <button
                    ...
                    <div className="flex items-center gap-2">
                      {linkingNumberId === number.id ? (
                        <div className="flex items-center gap-2 px-4 py-2.5 bg-purple-500/10 border border-purple-500/20 rounded-xl">
                          ...
                          <span className="text-sm text-gray-900 dark:text-white">Linking...</span>
                        </div>
