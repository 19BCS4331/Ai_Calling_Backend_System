      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-sm text-white/40">{totalMinutes} min total</span>
          </div>
                    ...
                    <div key={day.date} className="flex-1 group relative flex flex-col items-center justify-end h-full">
                      ...
                      <div className="absolute bottom-full mb-2 hidden group-hover:block z-10">
                        <div className="bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-xs whitespace-nowrap shadow-xl">
                          ...
                          <p className="text-white/60">{day.total_calls} calls · {day.total_minutes} min</p>
                        </div>
                      </div>
                    </div>
                  );
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Daily Cost</h2>
            <span className="text-sm text-white/40">${totalUserCost.toFixed(2)} total</span>
          </div>
          {filteredDaily.length === 0 ? (
            <div className="h-48 flex items-center justify-center text-white/30 text-sm">
              No data for this period
                  ...
                  return (
                    <div key={day.date} className="flex-1 group relative flex flex-col items-center justify-end h-full">
                      ...
                      <div className="absolute bottom-full mb-2 hidden group-hover:block z-10">
                        <div className="bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-xs whitespace-nowrap shadow-xl">
                          <p className="text-white font-medium">{day.date}</p>
                          <p className="text-white/60">${(day.total_cost_cents / 100).toFixed(2)} cost</p>
                        </div>
                      </div>
                    </div>
                  );
                ...
                <div key={item.label} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="text-white font-medium">{item.count} <span className="text-white/40 text-xs">({pct}%)</span></span>
                    </div>
        ...
        >
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
              <p className="text-sm text-white/50 mb-1">Avg Call Duration</p>
              <p className="text-xl font-bold text-white">
                {Math.floor(avgDuration / 60)}:{(avgDuration % 60).toString().padStart(2, '0')}
              </p>
            </div>
            <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
              <p className="text-sm text-white/50 mb-1">Total Minutes</p>
              <p className="text-xl font-bold text-white">{totalMinutes.toLocaleString()}</p>
            </div>
            <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
              <p className="text-sm text-white/50 mb-1">Completed Calls</p>
              <p className="text-xl font-bold text-white">{aggregates?.completedCalls.toLocaleString() || 0}</p>
            </div>
            <div className="p-4 bg-white/[0.02] border border-white/5 rounded-xl">
              <p className="text-sm text-white/50 mb-1">Failed Calls</p>
              <p className="text-xl font-bold text-white">{aggregates?.failedCalls.toLocaleString() || 0}</p>
            </div>
          </div>
        </motion.div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/5 bg-white/[0.02]">
                  ...
                  <th className="text-left text-xs font-medium text-white/40 uppercase tracking-wider p-4">Calls</th>
                  <th className="text-left text-xs font-medium text-white/40 uppercase tracking-wider p-4">Minutes</th>
                  <th className="text-left text-xs font-medium text-white/40 uppercase tracking-wider p-4">Web</th>
                  <th className="text-left text-xs font-medium text-white/40 uppercase tracking-wider p-4">Inbound</th>
              ...
              <tbody>
                {[...filteredDaily].reverse().map((day) => (
                  <tr key={day.date} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4 text-white font-medium">{day.total_calls}</td>
                    <td className="p-4 text-white/60">{day.total_minutes}</td>
                    <td className="p-4 text-blue-400">{day.web_calls ?? '—'}</td>
                    <td className="p-4 text-green-400">{day.inbound_calls ?? '—'}</td>
                    <td className="p-4 text-white/60 font-mono">${(day.total_cost_cents / 100).toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
